...
export default class ObjectPicker {
    constructor(renderingContext, context) {
        ...
        this.renderingContext.containerElement.addEventListener(
            "touchstart",
            this.onMouseDown
        )
        this.renderingContext.containerElement.addEventListener(
    ...
    @autobind
    onMouseUp(e) {
        this.context.globalEmitter.emit(
            EventTypes.WALL_CLICKED,
            this.getIntersection(e)
        )
    }
    ...
    getIntersection(e) {
        ...
        this.mouse.y =
            -(e.layerY / this.renderingContext.height) * 2 + 1
        ...
        const perspectiveIntersects = this.raycaster.intersectObjects(
            this.renderingContext.scene.children,
            true
        )

        const meshes = [...perspectiveIntersects].sort(
    ...
    destroy() {
        this.renderingContext.containerElement.removeEventListener(
            "touchstart",
            this.onMouseDown
        )
        this.renderingContext.containerElement.removeEventListener(
