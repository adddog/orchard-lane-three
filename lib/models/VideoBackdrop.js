...
export default class VideoBackdrop extends BaseModel {
    ...
    makeObject3D() {
        ...
        this.videoTexture.format = Device.isSafari
            ? RGBAFormat
            : RGBFormat
    ...
    getHLSMaterial(videoTexture) {
        ...
        const colorOrderOutput = Device.isMobileSafari
            ? "bgra"
            : "rgba"
    ...
    drawScreenShot() {
        const dataURI = this.context.store.getState().configuration
            .screenshotURI
        ...
        this.screenShotEl.onload = () => {
            ...
            if (typeof window.orientation === "undefined") {
                ...
                const phoneRotatedAfterScreenshot =
                    isDeviceLandscape !== isImageLandscape
        ...
        this.config.videoEl.addEventListener(
            "loadeddata",
    ...
    onVideoLoadedData() {
        ...
        this.config.videoEl.removeEventListener(
            "loadeddata",
            this.onVideoLoadedData
        )
    ...
    addRotation(vector) {
        let { x, y, z } = this.object3D.rotation
        x += vector.x
        y += vector.y
        z += vector.z
