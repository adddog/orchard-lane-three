...
import { setPlaybackTime,updateScene } from "./data/actions/sceneActions"
...
export default class VideoScene {
    ...
    updateScene(config) {
        this.store.dispatch(updateScene(config))
    }
    ...
    initialize() {
        if (!this.initialized) {
            ...
            this.objectPicker = new ObjectPicker(this.renderingContext,context)
        }
    }
